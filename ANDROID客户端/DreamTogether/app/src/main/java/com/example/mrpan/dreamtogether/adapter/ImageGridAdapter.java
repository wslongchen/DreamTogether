			...
			public void onClick(View v) {
				...
				if ((BitmapUtils.drr.size() + selectTotal) < 9) {
					...
					if (item.isSelected) {
						...
						if(isHave)
						{
							BitmapUtils.drr.remove(path);
							//MyLog.i("adapter", "removed:" + path + ",image size:" + BitmapUtils.drr.size());
						}
						map.put(path, path);
					...
					if (item.isSelected == true) {
						...
						holder.selected.setImageResource(0);
							selectTotal--;
						map.remove(path);
						MyLog.i("adapter", "removed2:" + path + ",image size:" + BitmapUtils.drr.size());

					} else {
						Message message = Message.obtain(mHandler, 0);
						message.sendToTarget();
						MyLog.i("adapter", "other:" + path + ",image size:" + BitmapUtils.drr.size());
					}
				}
			}
