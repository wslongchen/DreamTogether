...
import com.example.mrpan.dreamtogether.adapter.FaceGVAdapter;
import com.example.mrpan.dreamtogether.gif.AnimatedGifDrawable;
import com.example.mrpan.dreamtogether.gif.AnimatedImageSpan;
...
public class ExpressionUtils {
	...
	public static SpannableStringBuilder getFace(Context mContext,String png) {
		...
		try {
			...
			sb.append(tempText);
			sb.setSpan(
					new ImageSpan(mContext, bitmap), sb.length()
			...
			public void onItemClick(AdapterView<?> parent, View view,int position, long id) {
				try {
					...
					if (!png.contains("_del")) {// 如果不是删除图标
						ExpressionUtils.insert(editText,ExpressionUtils.getFace(context,png));
					} else {
						ExpressionUtils.delete(editText);
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
