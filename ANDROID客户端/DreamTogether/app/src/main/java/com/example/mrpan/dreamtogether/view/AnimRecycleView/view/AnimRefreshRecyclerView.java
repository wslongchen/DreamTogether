package com.example.mrpan.dreamtogether.view.AnimRecycleView.view;
 ...
 */
public class AnimRefreshRecyclerView extends RecyclerView implements Runnable{

    private Context mContext;

    private static AnimRefreshRecyclerView rfView;
    ...
    public AnimRefreshRecyclerView(Context context) {
        this(context, null);
    }

    public AnimRefreshRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AnimRefreshRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setHeaderImage(ImageView headerImage) {
        ...
        headerImageHeight = AnimRefreshRecyclerView.this.headerImage.getHeight();
        // 防止第一次拉伸的时候headerImage.getLayoutParams().height = 0
        if (headerImageHeight <= 0) {
            headerImageHeight = AnimRefreshRecyclerView.this.headerImage.getLayoutParams().height;
        } else {
...
//            View headerView = LayoutInflater.from(getContext()).inflate(R.layout.recyle_header_view, null);
//            headerImage=(ImageView)headerView.findViewById(R.id.iv_hander);
//            headerImage.setImageResource(R.drawable.refreshanim);
//            AnimationDrawable animationDrawable=(AnimationDrawable) headerImage.getDrawable();
//            animationDrawable.start();
//            headerImage.setMaxHeight(dp1 * 130);
//            headerLayout.removeView(headerImage);
...
//            mHeaderViews.add(0, headerLayout);
    ...
    public void run() {
        ...
        if (manager instanceof AnimRefreshLinearLayoutManager) {
            // ListView布局
            ((AnimRefreshLinearLayoutManager) manager).setOverScrollListener(mOverScrollListener);
        } else if (manager instanceof AnimRefreshGridLayoutManager) {
            layoutGridAttach((AnimRefreshGridLayoutManager) manager);
        } else if (manager instanceof AnimRefreshStaggeredGridLayoutManager) {
            layoutStaggeredGridHeadAttach((AnimRefreshStaggeredGridLayoutManager) manager);
        }

        if((WrapAdapter)mAdapter!=null){
     ...
     * @param manager
     */
    private void layoutStaggeredGridHeadAttach(AnimRefreshStaggeredGridLayoutManager manager) {
     ...
     * @param manager
     */
    private void layoutGridAttach(final AnimRefreshGridLayoutManager manager) {
        // GridView布局
        manager.setOverScrollListener(mOverScrollListener);
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadDataListener != null && !isLoadingData) {
            ...
            // 获取最后一个正在显示的Item的位置
            if (layoutManager instanceof AnimRefreshGridLayoutManager) {
                lastVisibleItemPosition = ((AnimRefreshGridLayoutManager) layoutManager).findLastVisibleItemPosition();
            } else if (layoutManager instanceof AnimRefreshStaggeredGridLayoutManager) {
                int[] into = new int[((AnimRefreshStaggeredGridLayoutManager) layoutManager).getSpanCount()];
                ((AnimRefreshStaggeredGridLayoutManager) layoutManager).findLastVisibleItemPositions(into);
                lastVisibleItemPosition = findMax(into);
            } else {
                lastVisibleItemPosition = ((AnimRefreshLinearLayoutManager) layoutManager).findLastVisibleItemPosition();
            }
...
//            ((ViewGroup) mHeaderViews.get(0)).addView(rfAnimView, params);
            ...
            //View headerView = LayoutInflater.from(mContext).inflate(R.layout.recyle_header_view, null);
            rfAnimView=new ImageView(mContext);
            rfAnimView.setImageResource(R.drawable.refreshanim);
